	...
	if ($unit_kode=='') {
		...
		if ($cek_bid > 0) {
			while ($b=$sql_list_bidang->fetch_object()) {
				if ($i_bid>1)  echo '<tr class="success"><td colspan="6"></td></tr>';
				$sql_unit_es4='';
				$unit_es3=$b->unit_kode;
				echo '<tr><td colspan="6"><strong>'.$b->unit_nama .'</strong></td></tr>';
				$i=1;
				$sql_keg_es4= $conn->query("select * from kegiatan,keg_target where kegiatan.keg_id=keg_target.keg_id and keg_t_unitkerja='$unit_es3' and keg_t_target>0");
		...
		$i=1;
				$sql_keg_es4= $conn->query("select * from kegiatan,keg_target where kegiatan.keg_id=keg_target.keg_id and keg_t_unitkerja='$unit_kode' and keg_t_target>0");
				while ($k=$sql_keg_es4->fetch_object()) {
					$target_total=$k->keg_t_target;
					//get_keg_kabkota_realisasi($keg_id,$unit_kabkota, $keg_jenis)
					$total_kirim=get_keg_kabkota_realisasi($k->keg_id,$k->keg_t_unitkerja,1);
