...
if ($_SESSION['sesi_level'] > 1) {
   ...
   if ($_SESSION['sesi_level']==2) {
   	$spj_d_unitkerja=$_SESSION['sesi_unitkerja'];
   }
   else {
...
if ($cek>0) {
   ...
   ?>
   <legend>Konfirmasi Pengiriman SPJ</legend>
   		<form id="formKirimTarget" name="formKirimTarget" action="<?php echo $url.'/'.$page;?>/savekirimspj/"  method="post" class="form-horizontal well" role="form">
   		<fieldset>
   		...
   		<div class="form-group">
   			<label for="keg_unitkerja" class="col-sm-2 control-label">Unit Kerja</label>
   				<div class="col-sm-6">
   					<?php echo '['.$kk->keg_s_unitkerja.'] '.$kk->unit_nama; ?>
   				</div>
   		</div>
			<div class="form-group">
   			<label for="keg_s_target" class="col-sm-2 control-label">Target</label>
   				<div class="col-sm-6">
   					<?php echo $kk->keg_s_target; ?> SPJ
   				</div>
   		</div>
   		<div class="form-group">
