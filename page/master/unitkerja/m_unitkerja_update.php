...
if ($_POST['submit_unit']) {
	...
	if ($cek>0) {
		 if ($unit_parent==NULL) {
			$sql_unit_save = $conn -> query("update unitkerja set unit_nama='$unit_nama',unit_diupdate_oleh='$created',unit_jenis='$unit_jenis', unit_eselon='$unit_eselon' where unit_kode='$unit_kode'");
		 }
		 else {
			 $sql_unit_save = $conn -> query("update unitkerja set unit_nama='$unit_nama',unit_diupdate_oleh='$created',unit_jenis='$unit_jenis',unit_parent='$unit_parent',unit_eselon='$unit_eselon' where unit_kode='$unit_kode'");
		 }
